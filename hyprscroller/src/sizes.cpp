...
StandardSize ScrollerSizes::get_single_row_height() {
  update();
...
Mode ScrollerSizes::get_mode(PHLMONITOR monitor) {
  update();
  // Helper: orientation-derived mode
  auto deriveByOrientation = [&](PHLMONITOR mon) -> Mode {
    // Prefer transformed size which accounts for wl_output_transform
    const auto ts = mon->m_transformedSize;
    if (ts.y > ts.x)
      return Mode::Column; // portrait → stack in a column
    else
      return Mode::Row; // landscape → scroll by row (columns)
  };

  for (const auto &monitor_data : monitors) {
    if (monitor_data.name == monitor->m_name) {
      if (monitor_data.auto_by_orientation)
        return deriveByOrientation(monitor);
      return monitor_data.mode;
    }
  }
  // Fallback to global default (may also be auto-by-orientation)
  if (global_auto_by_orientation)
    return deriveByOrientation(monitor);
  return Mode::Row;
}
...
void ScrollerSizes::update() {
  ...
  static auto const *column_default_width_str =
      (Hyprlang::STRING const *)HyprlandAPI::getConfigValue(
          ...
          ->getDataStaticPtr();
  static auto const *single_column_width_str =
      (Hyprlang::STRING const *)HyprlandAPI::getConfigValue(
          PHANDLE, "plugin:scroller:single_column_width")
          ->getDataStaticPtr();
  static auto const *single_row_height_str =
      (Hyprlang::STRING const *)HyprlandAPI::getConfigValue(
          PHANDLE, "plugin:scroller:single_row_height")
          ->getDataStaticPtr();
  ...
  static auto const *monitor_modes_str =
      (Hyprlang::STRING const *)HyprlandAPI::getConfigValue(
          ...
          ->getDataStaticPtr();
  static auto *const *auto_by_orient_ptr =
      (Hyprlang::INT *const *)HyprlandAPI::getConfigValue(
          PHANDLE, "plugin:scroller:auto_mode_by_orientation")
          ->getDataStaticPtr();

  const int auto_by_orient_val = **auto_by_orient_ptr;
  ...
  single_column_width =
      get_size_from_string(*single_column_width_str, StandardSize::One);
  single_row_height =
      get_size_from_string(*single_row_height_str, StandardSize::One);
  ...
  str_single_row_height = *single_row_height_str;
  str_window_heights = *window_heights_str;
  str_column_widths = *column_widths_str;
  str_monitors = *monitor_modes_str;
  global_auto_by_orientation = auto_by_orient_val != 0;
  cached_auto_by_orientation = auto_by_orient_val;
        ...
        if (!modes.empty()) {
          ...
          while (std::getline(stream, monitor_mode, ',')) {
            ...
            if (pos != std::string::npos) {
              ...
              monitor_data.auto_by_orientation = global_auto_by_orientation;
                        ...
                        if (!option_data.empty()) {
                          if (option_name == "mode") {
                            if (option_data == "r" || option_data == "row") {
                              monitor_data.mode = Mode::Row;
                              monitor_data.auto_by_orientation = false;
                            } else if (option_data == "c" ||
                                       option_data == "col" ||
                                       option_data == "column") {
                              monitor_data.mode = Mode::Column;
                              monitor_data.auto_by_orientation = false;
                            } else if (option_data == "a" ||
                                       option_data == "auto") {
                              monitor_data.auto_by_orientation = true;
                            }
