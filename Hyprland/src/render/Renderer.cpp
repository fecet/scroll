...
void CHyprRenderer::renderWindow(PHLWINDOW pWindow, PHLMONITOR pMonitor, const Time::steady_tp& time, bool decorate, eRenderPassMode mode, bool ignorePosition, bool standalone) {
    ...
    float workspaceAlpha = PWORKSPACE && PWORKSPACE->m_forceRendering ? 1.f : PWORKSPACE->m_alpha->value();
    renderdata.fadeAlpha = pWindow->m_alpha->value() * (pWindow->m_pinned || USE_WORKSPACE_FADE_ALPHA ? 1.f : workspaceAlpha) *
